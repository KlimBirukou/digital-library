...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<?> handleNotFound(OrganizationNotFoundException exception) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(Map.of("error", "Organization not found", "details", exception.getMessage()));
    }
    ...
    public ResponseEntity<?> handleTypeMismatch(MethodArgumentTypeMismatchException exception) {
        if (exception.getRequiredType() == UUID.class) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(Map.of("error", "Incorrect UUID format", "details", exception.getName()));
        }
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(Map.of("error", "Incorrect parameter type", "details", exception.getMessage()));
    }
}
